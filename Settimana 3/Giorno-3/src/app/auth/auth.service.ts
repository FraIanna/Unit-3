...
export class AuthService {
  ...
  syncIsLoggeedIn: boolean = false;

  user$ = this.authSubject.asObservable();
  isLoggedIn$: Observable<boolean> = this.user$.pipe(
    map((user) => !!user),
    tap((user) => (this.syncIsLoggeedIn = user))
  );
  ...
  apiLoginUrl: string = 'http://localhost:3000/login';
  apiRegisterUrl: string = 'http://localhost:3000/register';

  register(newUser: Partial<iUser>): Observable<iAuthResponse> {
    return this.http.post<iAuthResponse>(this.apiRegisterUrl, newUser);
  }

  login(loginData: iAuthData): Observable<iAuthResponse> {
    return this.http
      .post<iAuthResponse>(this.apiLoginUrl, loginData)

      .pipe(
        tap((data) => {
          this.authSubject.next(data.user);
          localStorage.setItem('accessData', JSON.stringify(data)); //salvo in localstorage l'access token e l'utente
          this.autoLogout;
        })
      );
  }
